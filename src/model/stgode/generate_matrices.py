...
def generate_dtw_spa_matrix(
    ...
    data_file = "./datasets/{0}/data_in_{1}_out_{2}_rescale_{3}.pkl".format(
        dataset_name, in_len, out_len, re_scale
    )
    with open(data_file, "rb") as f:
        data = pickle.load(f)["processed_data"]
    num_node = data.shape[1]
    if not os.path.exists(
        "{0}/{1}_dtw_distance.npy".format(
            os.path.abspath(__file__ + "/.."), dataset_name
        )
    ):
    ...
    if dataset_name in ["PEMS03", "PEMS04", "PEMS07", "PEMS08"]:
        if not os.path.exists(
            "{0}/{1}_spatial_distance.npy".format(
                os.path.abspath(__file__ + "/.."), dataset_name
            )
        ):
...
if __name__ == "__main__":
    ...
    generate_dtw_spa_matrix("PEMS07", 12, 12, re_scale=True)
