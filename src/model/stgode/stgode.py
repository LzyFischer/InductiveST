...
from ...lib.utils import DataTrimer_STGODE
...
class STGCNBlock(nn.Module):
    ...
    def forward(self, X):
        ...
        t = t.permute(0, 3, 1, 2)
        t = self.batch_norm(t).permute(0, 2, 3, 1)
...
class STGODE(nn.Module):
    ...
    def __init__(
        ...
        data_trimer = DataTrimer_STGODE(cfg)
        num_nodes_list, nodes_list, se_list, sp_list = data_trimer()
    ...
    def forward(
        ...
        """
        Args:
            x : input data of shape (batch_size, num_nodes, num_timesteps, num_features) == (B, N, T, F) (BNCL)
        Returns:
            prediction for future of shape (batch_size, num_nodes, num_timesteps_output)
        """
        x = history_data.permute(0, 1, 3, 2)  # (B, N, C, L) -> (B, L, N, C)
        outs = []
        # spatial graph
