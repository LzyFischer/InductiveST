...
class dualEncoder(nn.Module):
    ...
    def forward(self, xl, xh, te):
        ...
        spa_statesl = self.ssal(
            xl,
            self.adj.to(xl.device),
            self.eigvec.to(xl.device),
            self.eigvalue.to(xl.device),
        )
        spa_statesh = self.ssah(
            xh,
            self.adj.to(xl.device),
            self.eigvec.to(xl.device),
            self.eigvalue.to(xl.device),
...
class STWave(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.training:
            label_yl, _ = disentangle(
                future_data[..., 0:1].cpu().numpy(), self.wt, self.wl
            )
