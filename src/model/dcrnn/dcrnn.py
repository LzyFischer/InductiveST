...
class DCRNN(torch.nn.Module):
    ...
    def __init__(
        ...
        self.test_num_nodes = num_nodes_list[2]
        self._num_units = num_units
        self._max_diffusion_step = max_diffusion_step
        self._use_gc_for_ru = use_gc_for_ru
        # for support in supports:
        # self._supports.append(self._build_sparse_matrix(support))
        self._supports_train = [adj_mx_list[0]]
        self._supports_val = [adj_mx_list[1]]
        self._supports_test = [adj_mx_list[2]]
    ...
    def forward(self, inputs, hx):
        if self.training:
            self._num_nodes = self.train_num_nodes
            self._supports = self._supports_train
        elif self.val_num_nodes == inputs.size(2):
            self._num_nodes = self.val_num_nodes
            self._supports = self._supports_val
        else:
            self._num_nodes = self.test_num_nodes
            self._supports = self._supports_test
