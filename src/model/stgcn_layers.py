...
class ChebGraphConv(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.configs["no_graph"]:
            self.gso_train = torch.zeros(len(self.gso_train), len(self.gso_train))
            self.gso_val = torch.zeros(len(self.gso_val), len(self.gso_val))
            self.gso = torch.zeros(len(self.gso), len(self.gso))
        ...
        if self.training:
            ...
            if self.Ks - 1 < 0:
                ...
                x_0 = x
                x_1 = torch.einsum("hi,btij->bthj", self.gso_train, x)
                x_list = [x_0, x_1]
            elif self.Ks - 1 >= 2:
                x_0 = x
                x_1 = torch.einsum("hi,btij->bthj", self.gso_train, x)
                ...
                for k in range(2, self.Ks):
                    x_list.append(
                        torch.einsum("hi,btij->bthj", 2 * self.gso_train, x_list[k - 1])
                        - x_list[k - 2]
                    )
        elif self.val:
            # pdb.set_trace()
            ...
            if self.Ks - 1 < 0:
                ...
                x_0 = x
                x_1 = torch.einsum("hi,btij->bthj", self.gso_val, x)
                x_list = [x_0, x_1]
            elif self.Ks - 1 >= 2:
                x_0 = x
                x_1 = torch.einsum("hi,btij->bthj", self.gso_val, x)
                ...
                for k in range(2, self.Ks):
                    x_list.append(
                        torch.einsum("hi,btij->bthj", 2 * self.gso_val, x_list[k - 1])
                        - x_list[k - 2]
                    )
        else:
...
class GraphConv(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            first_mul = torch.einsum("hi,btij->bthj", self.gso_train, x)
            second_mul = torch.einsum("bthi,ij->bthj", first_mul, self.weight)
        elif self.val:
            self.gso_val = self.gso_val.to(x.device)

            first_mul = torch.einsum("hi,btij->bthj", self.gso_val, x)
            second_mul = torch.einsum("bthi,ij->bthj", first_mul, self.weight)
        else:
            self.gso = self.gso.to(x.device)
...
class STConvBlock(nn.Module):
    ...
    def __init__(
        ...
        self.tc2_ln_train = nn.LayerNorm(
            [n_vertex_train, channels[2]], elementwise_affine=False
        )
...
class OutputBlock(nn.Module):
    ...
    def __init__(
        ...
        self.tc1_ln_train = nn.LayerNorm(
            [n_vertex_train, channels[0]], elementwise_affine=False
        )
