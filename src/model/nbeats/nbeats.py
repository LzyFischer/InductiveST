...
class NBeats(t.nn.Module):
    ...
    def __init__(self, cfg: dict, **kwargs):
        ...
        input_size = cfg["model"]["window_size"] - cfg["model"]["horizon"]
        output_size = cfg["model"]["horizon"]
        type = cfg["model"]["type"]
    ...
    def forward(
        self, history_data: t.Tensor, future_data: t.Tensor, **kwargs
    ) -> t.Tensor:
        B, N, C, L = history_data.shape
        history_data = history_data[:, :, [0]].transpose(2, 3)  # [B, N, L, 1]
        ...
        forecast = forecast.reshape(B, N, forecast.shape[-1])
